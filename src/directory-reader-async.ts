 ...
 * @param {Set<string>} visitedDirectoryPaths - Real directory paths already scanned during this traversal.
...
export default async function readDirectoryAsync(
  ...
  const realTargetDirectoryPath = await fs.realpath(targetDirectoryPath);
  ...
  const result = await Promise.all(
    receivedItemsPaths.map(async (itemPath) => {
      ...
      if (stat.isDirectory() && options.includeSubdirectories) {
        const files = await readDirectoryAsync(options, relativeItemPath, visitedDirectoryPaths);
