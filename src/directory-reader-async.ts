 ...
 * @param {Set<string>} visitedDirectoryRealPaths - The real paths already scanned in this traversal.
...
export default async function readDirectoryAsync(
  ...
  visitedDirectoryRealPaths = new Set<string>(),
): Promise<string[]> {
  const targetDirectoryRealPath = await fs.realpath(targetDirectoryPath);

  if (visitedDirectoryRealPaths.has(targetDirectoryRealPath)) {
    return [];
  }

  visitedDirectoryRealPaths.add(targetDirectoryRealPath);
