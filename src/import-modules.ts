...
/**
 * Remove a module and its dependency tree from Node.js require cache.
 * @param {string} modulePath - The absolute path of the module to invalidate.
 * @param {Set<string>} visitedModuleIds - Internal set used to avoid cycles.
 * @returns {void}
 */
function clearRequireCacheRecursively(
  modulePath: string,
  visitedModuleIds: Set<string> = new Set<string>(),
): void {
  ...
  if (visitedModuleIds.has(resolvedModulePath)) return;

  visitedModuleIds.add(resolvedModulePath);
  ...
  for (const childModule of cachedModule.children) {
    clearRequireCacheRecursively(childModule.id, visitedModuleIds);
  }
...
function importModule(
  ...
  if (options.forceReload) {
    clearRequireCacheRecursively(filePath);
  }
