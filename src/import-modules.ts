...
/**
 * Check whether the file is the caller module while Node is still loading it.
 * @param {string} filePath - The file path currently considered for import.
 * @param {string} callerFilePath - The caller file path detected from the stack.
 * @returns {boolean} Whether importing the file would re-enter the active caller module.
 */
function isCurrentlyLoadingCallerModule(filePath: string, callerFilePath: string): boolean {
  if (path.resolve(filePath) !== path.resolve(callerFilePath)) {
    return false;
  }
...
function importModule(
  ...
  if (isCurrentlyLoadingCallerModule(filePath, options.callerFilePath)) return false;
