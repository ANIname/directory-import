...
function syncHandler(options: ImportedModulesPrivateOptions): ImportedModules {
  ...
  const importableFilePaths = getImportableFilePaths(filesPaths, options);

  if (options.forceReload) {
    deleteModulesFromRequireCache(importableFilePaths);
  }

  let index = 0;

  for (const filePath of importableFilePaths) {
...
/**
 * Get the file paths that can be imported by directory-import.
 * @param {string[]} filesPaths - The file paths discovered in the target directory.
 * @param {ImportedModulesPrivateOptions} options - The private options generated by the preparePrivateOptions function.
 * @returns {string[]} File paths that match the supported extensions and import pattern.
 */
function getImportableFilePaths(filesPaths: string[], options: ImportedModulesPrivateOptions): string[] {
...
/**
 * Delete modules from Node's require cache before importing them again.
 * @param {string[]} filePaths - The file paths to remove from the require cache.
 * @returns {void}
 */
