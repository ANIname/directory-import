...
/**
 * Remove a module from `require.cache` and from its parent's `children` list.
 * Deleting the cache entry alone leaves the previous `Module` instance reachable
 * through `module.children`, so `forceReload` would retain old exports forever.
 * @param {string} filePath - Absolute path of the module to evict.
 * @returns {void}
 */
function evictCachedModule(filePath: string): void {
  ...
  if (cachedModule?.parent?.children) {
    const cachedModuleIndex = cachedModule.parent.children.indexOf(cachedModule);

    if (cachedModuleIndex !== -1) {
      cachedModule.parent.children.splice(cachedModuleIndex, 1);
    }
  }
