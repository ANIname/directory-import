...
async function asyncHandler(options: ImportedModulesPrivateOptions): Promise<ImportedModules> {
  ...
  for (const filePath of filesPaths) {
    const isModuleImported = await importModuleAsync(filePath, index, options, modules);
...
/**
 * Async version of importModule that properly handles async callbacks.
 * @param {string} filePath - The path to the file to import.
 * @param {number} index - The index of the module.
 * @param {ImportedModulesPrivateOptions} options - The private options generated by the preparePrivateOptions function.
 * @param {ImportedModules} modules - The object containing all imported modules.
 * @returns {Promise<boolean>} Whether the module was imported or not.
 */
async function importModuleAsync(
  filePath: string,
  index: number,
  options: ImportedModulesPrivateOptions,
  modules: ImportedModules,
  ...
  if (options.callback) {
    // Call the callback and await if it returns a promise to ensure proper async behavior
    try {
      ...
      // Continue processing other modules even if callback fails
