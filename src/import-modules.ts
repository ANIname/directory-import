...
function syncHandler(options: ImportedModulesPrivateOptions): ImportedModules {
  ...
  const importableModules = getImportableModules(filesPaths, options);

  if (options.forceReload) {
    clearRequireCache(importableModules);
  }

  for (const [index, importableModule] of importableModules.entries()) {
    importModule(importableModule, index, options, modules);
  }
...
/**
 * Get the files that should be imported according to the provided options.
 * @param {string[]} filesPaths - The discovered file paths.
 * @param {ImportedModulesPrivateOptions} options - The private options generated by the preparePrivateOptions function.
 * @returns {ImportableModule[]} The importable modules with their derived metadata.
 */
function getImportableModules(filesPaths: string[], options: ImportedModulesPrivateOptions): ImportableModule[] {
  const importableModules: ImportableModule[] = [];

  for (const filePath of filesPaths) {
    const importableModule = getImportableModule(filePath, options);

    if (!importableModule) continue;

    importableModules.push(importableModule);

    if (importableModules.length === options.limit) break;
  }
...
/**
 * Get import metadata for a file if it can be imported.
 * @param {string} filePath - The path to the file to inspect.
 * @param {ImportedModulesPrivateOptions} options - The private options generated by the preparePrivateOptions function.
 * @returns {ImportableModule | undefined} The import metadata, or undefined when the file should be skipped.
 */
function getImportableModule(filePath: string, options: ImportedModulesPrivateOptions): ImportableModule | undefined {
  ...
  if (!isValidModuleExtension) return undefined;
  if (!isValidFilePath) return undefined;
  if (isDeclarationFile) return undefined;

  return {
    fileName,
    filePath,
    relativeModulePath: filePath.slice(options.targetDirectoryPath.length),
  };
}

/**
 * Clear Node's require cache for all modules before importing any of them.
 * @param {ImportableModule[]} importableModules - The modules whose cache entries should be removed.
 * @returns {void}
 */
function clearRequireCache(importableModules: ImportableModule[]): void {
  for (const { filePath } of importableModules) {
 ...
 * @param {ImportableModule} importableModule - The module metadata to import.
...
function importModule(
  importableModule: ImportableModule,
  index: number,
  options: ImportedModulesPrivateOptions,
  modules: ImportedModules,
): void {
  const { fileName, filePath, relativeModulePath } = importableModule;
