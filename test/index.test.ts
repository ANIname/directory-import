...
/**
 * Creates a temporary fixture where a directory symlink points back to its parent directory.
 * @returns {string} Path to the temporary fixture directory.
 */
const createDirectoryWithRecursiveSymlink = (): string => {
  const temporaryDirectoryPath = fs.mkdtempSync(path.join(os.tmpdir(), 'directory-import-recursive-link-'));

  fs.writeFileSync(path.join(temporaryDirectoryPath, 'module.js'), 'module.exports = { value: true };\n');
  fs.symlinkSync(temporaryDirectoryPath, path.join(temporaryDirectoryPath, 'self'), 'dir');
...
test('Import modules synchronously from a directory with a recursive symlink', () => {
  const temporaryDirectoryPath = createDirectoryWithRecursiveSymlink();
...
test('Import modules asynchronously from a directory with a recursive symlink', async () => {
  const temporaryDirectoryPath = createDirectoryWithRecursiveSymlink();
