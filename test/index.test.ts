...
test('Import modules when Error.prepareStackTrace returns a non-string stack', () => {
  const temporaryDirectoryPath = fs.mkdtempSync(path.join(os.tmpdir(), 'directory-import-stack-'));
  const moduleFilePath = path.join(temporaryDirectoryPath, 'sample.js');
  fs.writeFileSync(moduleFilePath, 'module.exports = { ok: true };\n');
  ...
  try {
    ...
    expect(result).toEqual({ '/sample.js': { ok: true } });
  } finally {
